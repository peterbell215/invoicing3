...
describe 'Client' do
  ...
  describe '#current_rate' do
    context 'when a new record is built with a nil value for hourly_charge' do
      ...
      it 'autofills the meeting_charge.' do
    ...
    context 'when a new record is created with a nil value for hourly_charge' do
      ...
      it 'creates a child record meeting_charge with a default value' do
  ...
  describe '#current_rate=' do
    context 'when a new client record is built with current_rate set' do
      ...
      it 'creates a corresponding meeting_charge child' do
    ...
    context 'when the same current_rate is set a 2nd time' do
      ...
      it 'does not do an update to the current meeting_charge record' do
        test_client.current_rate = Money.new(7000)
        expect(test_client.current_meeting_charge.hourly_charge_rate_pence_changed?).to be_falsey
      end
    end
  end

  describe 'ensure no overlaps on meeting_charges' do
    context 'when non-overlapping meeting_charges exist' do
      subject(:test_client) { build(:client_with_meeting_charges) }
    ...
    context 'when overlapping meeting_charges exist' do
      subject(:test_client) { build(:client_with_meeting_charges, gap: -5.days) }

      it 'fails validation' do
        expect(test_client).not_to be_valid
        expect(test_client.errors[:meeting_charges].first)
          .to eq 'meeting charge to 2023-06-01 overlaps with its successor'
      end
    end
  end
end
